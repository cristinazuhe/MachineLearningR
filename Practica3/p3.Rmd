...
//COMNETARRRRRRRRRRRRR. ¿FALLA ALGO EN KNN????ERROR PARECE MUY GRANDE.....
```{r}
...
Vamos a cargar la base de tantos y las librerías que usaremos para los próximos apartados. Ahora, al igual que hacíamos en el ejercicio1, consideramos un 80% de los datos de Boston y los almacenamos en BostonTrain. Los datos restantes los guardamos en BostonTest
```{r}
...
Vemos que efectivamente la variable medv está en nuestro conjunto de datos y ajustamos el modelo de regresión usando bagging.  //EXPLICAR LO DEL LIBRO 
```{r}
 names(Boston)
 boston_bagging = randomForest( medv~.,data=Boston , subset=indices_train ,mtry=13,importance =TRUE)
 boston_bagging
```

```{r}
 yhat.bag = predict (boston_bagging , Boston_test)
